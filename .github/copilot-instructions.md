...
1. **Дашборд студента** (`/`, `pages/Index.tsx`) - Основной интерфейс обучения
   - Навигация на основе state: home → комната модуля → урок → завершение
   - Модули из локального массива `modules` с отслеживанием прогресса
   - Уроки из `COMPLETE_LESSONS` в `data/allLessonsData.ts` (44 урока)
   - Колесо баланса при первом запуске (проверка localStorage: `initialBalanceScores`)
